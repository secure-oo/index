...
function loadState() {
  ...
  panelOpen  = ls(KEY.panel) !== false; // 默认展开
}
...
function sendToChat() {
  const moves = log
    .filter(l => l.text.includes('走了'))
    .map(l => {
      const m = l.text.match(/([a-h][1-8]-[a-h][1-8])/i);
      return m ? (l.who==='你'?'白':l.who==='爸爸'?'黑':'?')+':'+m[1] : null;
    })
    .filter(Boolean);
  if (!moves.length) return;
  const text = '📋 棋步记录：' + moves.join(' · ');
  // 兼容 SillyTavern 主流版本
  const ta = document.querySelector('#send_textarea') || document.querySelector('textarea#send_textarea');
  if (ta) {
    const prev = ta.value;
    ta.value = prev ? prev + '\n' + text : text;
...
function buildWin() {
  ...
  <div id="chess-body">
    ...
    <button id="chess-panel-toggle">
      ...
      <span id="chess-panel-toggle-label">详情</span>
    </button>
    ...
    <div id="chess-panel">
      ...
      <div id="chess-log-sec">
        <div id="chess-log-header">
          ...
          <button id="chess-send-btn">发送到聊天</button>
        </div>
  ...
  // 面板收起/展开
  document.getElementById('chess-panel-toggle').addEventListener('click', e=>{
    e.stopPropagation();
    panelOpen = !panelOpen;
    ls(KEY.panel, panelOpen);
    applyPanelToggle();
  });

  // 发送棋步到聊天
  document.getElementById('chess-send-btn').addEventListener('click', sendToChat);
